...
impl<'a, R: femtovg::Renderer + TextureImporter> ItemRenderer for GLItemRenderer<'a, R> {
    ...
    fn visit_clip(
        ...
        if !radius.is_zero() {
            ...
            {
                // rdb patch (see PATCHES.md): blit on whole pixels, matching
                // `render_into_layer`, so the texture is copied 1:1.
                let layer_origin = layer_origin.floor();
...
impl<'a, R: femtovg::Renderer + TextureImporter> LayerRenderer<'a> for GLItemRenderer<'a, R> {
    ...
    fn create_layer_target(
        ...
        // rdb patch (see PATCHES.md): the layer is drawn from its floored
        // origin, so it needs one spare pixel to hold the dropped fraction.
        if physical_size.is_empty() {
            return None;
        }
        let size = (physical_size + euclid::Size2D::new(1., 1.)).ceil().try_cast::<u32>()?;
    ...
    fn render_into_layer(
        ...
        // rdb patch (see PATCHES.md): render from a whole-pixel origin. At a
        // fractional scale factor the unsnapped origin made every blit of this
        // layer resample by a sub-pixel offset, blurring its contents.
        let physical_origin = (bounding_rect.origin.cast() * self.scale_factor).floor();
...
impl<'a, R: femtovg::Renderer + TextureImporter> GLItemRenderer<'a, R> {
    ...
    fn render_and_blend_layer(&mut self, alpha_tint: f32, item_rc: &ItemRc) -> RenderingResult {
        ...
        {
            // rdb patch (see PATCHES.md): same whole-pixel origin as the render.
            let layer_origin = layer_origin.floor();
